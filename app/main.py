...
def registrar(motor, modelo, entrada, salida, latencia_ms):
    ...
    with conexion() as con, con.cursor() as cur:
        cur.execute(
            "INSERT INTO inferencias (motor, modelo, entrada, salida, latencia_ms) VALUES (%s, %s, %s, %s, %s)",
...
def clasificar_ollama(texto: str) -> str:
    ...
    prompt = (
        f"Clasifica el siguiente mensaje de commit en UNA de estas categorias: {', '.join(TIPOS)}. "
...
def inferencias(limite: int = 20):
    ...
    with conexion() as con, con.cursor() as cur:
        cur.execute(
            "SELECT id, fecha, motor, modelo, entrada, salida, latencia_ms FROM inferencias ORDER BY id DESC LIMIT %s",
            (limite,),
        )
        filas = cur.fetchall()
